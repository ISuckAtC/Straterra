...
public class Grid : MonoBehaviour
{
    ...
    void Start()
    {
		...
		//UpdateValidTiles(GameManager.I.gridRange);

        onReady.Invoke();

        //PlaceTiles._instance.ColorAllTiles();

        PlaceTiles._instance.SetWaterEdgeTiles();
        PlaceTiles._instance.ColorTilesByResourceAmount();
    }

	public void CanSetValidTiles()
	{
		UpdateValidTiles(GameManager.I.gridRange);
	}
    ...
    public void UpdateValidTiles(GridRange[] gr)//int position, int radius)
    {
		GameManager.I.UpdateGridRange();
        validTiles = GetTilesWithinDistance(gr);
    }

    private int[] GetTilesWithinDistance(GridRange[] gr) //int position, int radius)
    {
        ...
        for (int x = 0; x < gr.Length; x++)
        {
            ...
            Vector2Int start = GetPosition(gr[x].position);
            int startwidth, startheight, endwidth, endheight;
            if (start.x - gr[x].radius < 0) startwidth = 0;
            else startwidth = start.x - gr[x].radius;
            if (start.x + gr[x].radius > width) endwidth = width;
            else endwidth = start.x + gr[x].radius;
            if (start.y - gr[x].radius < 0) startheight = 0;
            else startheight = start.y - gr[x].radius;
            if (start.y + gr[x].radius > height) endheight = height;
            else endheight = start.y + gr[x].radius;
            int valid = 0;
		...
		{
			if (e < inrange.Count - 1)
			{
				if (e == e+1)
					inrange.RemoveAt(e + 1);
			}
		}
